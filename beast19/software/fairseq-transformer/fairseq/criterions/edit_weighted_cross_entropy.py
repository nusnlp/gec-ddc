...
class EditWeightedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        net_output = model(**sample['net_input'])  #net_output[0] = BxTxV
        #print("NO",[no.size() for no in net_output])  # size
        lprobs = model.get_normalized_probs(net_output, log_probs=True) # BxTxV
        lprobs = lprobs.view(-1, lprobs.size(-1)) # BTxV
        #print("lprobs2",lprobs.size() )   # BTxV

        target = model.get_targets(sample, net_output).view(-1)   # BT
        ...
        try:
            ...
            print('| error: alignment did not match, probably validation (using normal cross entropy loss).')
            # weighted log probs is log probs itself (for validation and in cae of error)
            wlprobs = lprobs

        #print("ALIGN_BATCH", weighted_alignbatch)

        #print("UNWEIGHTED", lprobs)
        #print("WEIGHTED", lprobs * weighted_alignbatch.view(-1,1))
