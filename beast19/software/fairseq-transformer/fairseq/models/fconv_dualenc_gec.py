...
@register_model('fconv_dualenc_gec')
...
class FConvCustomDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, auxencoder_out_dict=None, encoder_out_dict=None, incremental_state=None):
        ...
        residuals = [x]
        for proj, conv, attention, auxattention, res_layer in zip(self.projections, self.convolutions, self.auxattention, self.attention, self.residuals):
            if res_layer > 0:
                residual = residuals[-res_layer]
                residual = residual if proj is None else proj(residual)
            else:
            ...
            if attention is not None:
                ...
                if acx is not None:
                    x = (x + acx) * math.sqrt(self.normalization_constant)
...
@register_model_architecture('fconv_dualenc_gec', 'fconv_dualenc_gec')
