...
class TransformerLM(FairseqModel):
...
#                                 ' (requires shared dictionary and embed dim)')
    ...
    def build_model(cls, args, task):
        ...
        #     if args.encoder_embed_path != args.decoder_embed_path:
        #         raise RuntimeError(
        #             '--share-all-embeddings requires --encoder-embed-path to match --decoder-embed-path')
       ...
       # else:
        ...
        encoder = TransformerLMEncoder(src_dict) # empty encoder

        decoder = TransformerLMDecoder(args, tgt_dict, decoder_embed_tokens)
        return TransformerLM(encoder, decoder)

class TransformerLMEncoder(FairseqEncoder):
    """Empty encoder"""
...
class TransformerLMDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=False):
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            TransformerLMDecoderLayer(args)
...
class TransformerLMDecoderLayer(nn.Module):
    ...
    def __init__(self, args):
        ...
        for param in self.layer_norms[1].parameters():
