...
@register_model('fconv_dualenc_gec_gated')
...
class Gating(nn.Module):
    ...
    def __init__(self, gate_dim, inputs_dim):
        ...
        self.auxattn_proj = Linear(inputs_dim, gate_dim)

    def forward(self, decoder_state, attn, auxattn):
        """ computes a gate vector from the provded inputs """

        gate = torch.sigmoid(self.decoder_state_proj(decoder_state) + self.attn_proj(attn) + self.auxattn_proj(attn))
        return gate
...
class FConvCustomDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.auxattention = nn.ModuleList() # adding auxiliary attention for the context
        ...
        for i, (out_channels, kernel_size, residual) in enumerate(convolutions):
            ...
            self.auxattention.append(AttentionLayer(out_channels, embed_dim, self.normalization_constant)
                                  if attention[i] else None) # aux attention
    ...
    def forward(self, prev_output_tokens, auxencoder_out_dict=None, encoder_out_dict=None, incremental_state=None):
        ...
        residuals = [x]
        for proj, conv, attention, auxattention, res_layer, auxgate in zip(self.projections, self.convolutions, self.auxattention, self.attention, self.residuals, self.auxgates):
            if res_layer > 0:
                residual = residuals[-res_layer]
                residual = residual if proj is None else proj(residual)
            else:
            ...
            if attention is not None:
                ...
                if acx is not None:
                    # find gate if auxiliary attention is present
                    auxgate_vector = auxgate(decoder_state=x, attn=cx, auxattn=acx)

                x =  (x + cx) * math.sqrt(self.normalization_constant)

                # add auxiliary attention
                if acx is not None:
                    x = (x + auxgate_vector * acx) * math.sqrt(self.normalization_constant)
...
@register_model_architecture('fconv_dualenc_gec_gated', 'fconv_dualenc_gec_gated')
