...
@register_model('transformer_dualenc_gec_gatedaux')
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x, auxencoder_out, auxencoder_padding_mask, encoder_out, encoder_padding_mask, incremental_state):
        ...
        gate = self.gating(residual, x_enc)
        x = residual + x_enc + gate*x_auxenc
        x = self.maybe_layer_norm(1, x, after=True)
...
class Gating(nn.Module):
    ...
    def __init__(self, gate_dim, inputs_dim):
        super().__init__()
        self.self_attn_proj = Linear(inputs_dim, gate_dim)
        self.encoder_attn_proj = Linear(inputs_dim, gate_dim)

    def forward(self, residual, x_enc):
        """ computes a gate vector from the provded inputs """
        gate = torch.sigmoid(self.self_attn_proj(residual) + self.encoder_attn_proj(x_enc))
        return gate
...
@register_model_architecture('transformer_dualenc_gec_gatedaux', 'transformer_dualenc_gec_gatedaux')
