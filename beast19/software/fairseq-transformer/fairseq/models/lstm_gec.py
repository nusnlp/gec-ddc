...
from . import (
    FairseqEncoder, FairseqIncrementalDecoder, FairseqLanguageModel, FairseqModel, register_model,
    register_model_architecture,
)


@register_model('lstm_gec')
class LSTMCustomModel(FairseqModel):
...
class LSTMCustomEncoder(FairseqEncoder):
    """LSTM encoder."""
    def __init__(
...
class LSTMCustomLanguageModel(FairseqLanguageModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--dropout', default=0.1, type=float, metavar='D',
                            help='dropout probability')
        parser.add_argument('--embed-dropout', default=0.1, type=float, metavar='D',
                            help='dropout probability')
        parser.add_argument('--decoder-embed-dim', type=int, metavar='N',
                            help='decoder embedding dimension')
    ...
    def build_model(cls, args, task):
       ...
       # args.decoder_layers = eval(args.layers)
        #attention=eval(args.decoder_attention),
        layers=eval(args.decoder_layers)
        hidden_size=layers[0][0] # only take the first hidden layer dimension
        print(args)
        decoder = LSTMCustomDecoder(
            dictionary=task.target_dictionary,
            embed_dim=args.decoder_embed_dim,
            hidden_size=hidden_size,
        ...
        return LSTMCustomLanguageModel(decoder)


class LSTMCustomDecoder(FairseqIncrementalDecoder):
    """LSTM decoder."""
    def __init__(
        self, dictionary, max_positions=1024, embed_dim=512, hidden_size=512, out_embed_dim=512,
        num_layers=1, dropout_in=0.1, dropout_out=0.1, attention=True,
        encoder_embed_dim=512, encoder_output_units=512, positional_embeddings=True, pretrained_embed=None,
    ):
        super().__init__(dictionary)
        self.dropout_in = dropout_in
        self.dropout_out = dropout_out
        ...
        if hidden_size != out_embed_dim:
            self.fc2 = Linear(hidden_size, out_embed_dim)
        self.fc3 = Linear(out_embed_dim, num_embeddings, dropout=dropout_out)
...
@register_model_architecture('lstm_gec', 'lstm_gec')
...
@register_model_architecture('rnnlm', 'rnnlm')
def base_lm_architecture(args):
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 128)
    args.decoder_layers = getattr(args, 'decoder_layers', '[(1268, 4)] * 13')
    args.decoder_attention = getattr(args, 'decoder_attention', 'False')
    args.criterion = getattr(args, 'criterion', 'cross_entropy')
    args.dropout = getattr(args, 'dropout', 0.0)



@register_model_architecture('lstm_gec', 'lstm_wiseman_iwslt_de_en_gec')
...
@register_model_architecture('lstm_gec', 'lstm_luong_wmt_en_de_gec')
