...
class departureLossRL(nn.Module): # parsers
    def __init__(self, device, word_alphabet, vocab_size, args):
        ...
        departure_args_path = ['weiqi_single_args_3.pt', 'weiqi_single_args_4.pt']
    ...
    def forward(self, sel, pb, golden_out, mask_id, stc_length_out, sample_input, ignore_index=-100):
        ...
        for i in range(batch):  #batch
            ...
            for j in range(len(self.models)):
                ...
                # print('golden_out[i]:')
                # print(golden_out[i])
                # print('model1_preds[j][i]:')
                # print(model1_preds[j][i])
                reward = self.get_reward_departure(golden_out=golden_out[i], departure_out_list=model1_preds[j][i], ignore_index=-100)  #  we now only consider a simple case. the result of a third-party parser should be added here.
                rewards = rewards+reward
            rewards_z1.append(rewards)
