...
class load_weiqi_single(torch.nn.Module): # parsers
    def __init__(self, main_args, departure_args_path):
        ...
        # Load ensemble
        # main_model_paths = main_args.path.split(':')
        # main_models, main_model_args = utils.load_ensemble_for_inference(main_model_paths, main_task)
        # Set dictionaries
        main_src_dict = main_task.source_dictionary  # dic should be the same !! TODO:
        main_tgt_dict = main_task.target_dictionary
        # Optimize ensemble for generation
        # for main_model in main_models:
        #     main_model.make_generation_fast_(
        #         beamable_mm_beam_size=None if main_args.no_beamable_mm else main_args.beam,
        #     )
