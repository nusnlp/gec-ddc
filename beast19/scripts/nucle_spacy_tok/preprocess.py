...
class PreProcessor:
    ...
    def __init__(self):
        ...
        self.wordTokenizer = en_core_web_sm.load()
        ...
        for doc in docs:
            for par in doc.paragraphs:
                ...
                for s in self.sentenceTokenizer.tokenize(par):
                    doc.buildSentence(s, [], '', [], [], self.wordTokenizer)
        return docs
    ...
    def featureGeneration(self, docs, option):
        ...
        for doc in docs:
            for par in doc.paragraphs:
                doc.sentences.append([])
                for s in self.sentenceTokenizer.tokenize(par):
                    sent = nucle_sent(s, [], '', [], [], self.wordTokenizer)
    ...
    def tokenizeCorrectionStr(self, correctionStr, wdstart, wdend, words):
        ...
        if sentList != ['']:
            for sent in sentList:
                pseudoWordsList += [tok.text for tok in self.wordTokenizer.tokenizer(sent) if tok.text!=' ']
